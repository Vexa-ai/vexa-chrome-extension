...
import './ThreadDeletePromptModal.scss';
...
export function ThreadDeletePromptModal({ deleteThread }: ThreadDeletePromptModalProps) {
  ...
  useEffect(() => {
    const deleteThreadEditorCleanup = onMessage(MessageType.DELETE_THREAD_START, (message: { thread: Option }) => {
      setThreadData(message.thread);
      // setInitialThread(ThreadData.Thread || '');
      setShowEditorModal(true);
    });

    const deleteThreadEditorCompletedCleanup = onMessage(MessageType.DELETE_THREAD_COMPLETE, () => {
      closeModal();
    });
            ...
            <div className="w-full h-full flex items-center justify-center">
              <div className="ModalUi mx-3 p-4 rounded-lg w-full bg-[#0C111D]">
                ...
                <div className="flex flex-col w-full gap-2">
                  ...
                  <button onClick={closeModal} className="w-full p-2 rounded-md border border-[#333741] disabled:bg-[#1F242F] bg-[#161B26] text-[#CECFD2] text-center font-semibold text-base">
                    Cancel
                  </button>
                </div>
              </div>
